...
/* retourne le nombre de couples a l'interieur de la map */
T_MAP_INTERFACE size_t MAP_METHOD(length)(T map);
/* supprime la map passee en parametre (la libere proprement) */
T_MAP_INTERFACE void MAP_METHOD(delete)(T *map);
/* renvoie 1 si la map contient la cle, 0 sinon*/
T_MAP_INTERFACE int MAP_METHOD(contains)(T map, T_MAP_KEY key);
/* Supprime le couple de cle key dans la map */
...
/* Stocke la valeur de value de la cle si le couple est bien dans la map */
