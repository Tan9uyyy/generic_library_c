...
/* Retourne le nombre d'elements dans la stack */
T_STACK_INTERFACE int           STACK_METHOD(length) (T stack);
/* Retourne le premier element de la stack */
T_STACK_INTERFACE stack_datum_t STACK_METHOD(first) (T stack);
/* Insere l'element a la fin de la stack et retourne la nouvelle stack */
T_STACK_INTERFACE T             STACK_METHOD(push) (stack_datum_t value, T stack);
/* Retire le premier element de la stack, le stocke dans value et retourne la nouvelle stack */
T_STACK_INTERFACE T             STACK_METHOD(pop) (stack_datum_t *value, T stack);
/* Supprime la stack proprement en utilisant le destructeur sur chaque element */
T_STACK_INTERFACE void          STACK_METHOD(delete) (T stack);
/* Affiche la stack en utilisant la fonction printer pour chaque element */
