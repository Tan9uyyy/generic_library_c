...
int HEAP_METHOD(contains_rec)(T heap, heap_datum_t value, int index, int len) {
  if (HEAP_COMPARATOR(heap[index], value) > 0)
    return 0; // If root is bigger we stop
...
int HEAP_METHOD(push)(T *heap, heap_datum_t value) {
  ...
  while (HEAP_COMPARATOR((*heap)[index], (*heap)[FATHER(index)]) < 0 &&
         index != 0) {
...
int heap_smallest_son(T heap, int index) {
  ...
  if (LEFT_SON(index) >= len)
    return 0; // Case without any son
  if (RIGHT_SON(index) >= len)
    return LEFT_SON(index); // Case with only left son

  if (HEAP_COMPARATOR(heap[LEFT_SON(index)], heap[RIGHT_SON(index)]) >= 0)
...
int HEAP_METHOD(pop)(T *heap, heap_datum_t *value) {
  ...
  int index = 0;
  int smallest_son = heap_smallest_son(*heap, 0);
  while (smallest_son) {
    if (HEAP_COMPARATOR((*heap)[index], (*heap)[smallest_son]) < 0)
      break; // If smallest son bigger we stop swapping

    HEAP_METHOD(swap)(*heap, index, smallest_son);
    index = smallest_son;
    smallest_son = heap_smallest_son(*heap, index);
  }
...
void HEAP_METHOD(print)(T heap) {
  ...
  printf("{");
  HEAP_PRINTER(heap[0]); // Case of multiple elements
  for (int i = 1; i < len; i++) {
    printf(", ");
    HEAP_PRINTER(heap[i]);
  }
