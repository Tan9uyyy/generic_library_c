...
*/
TYPE(T, datum_t) METHOD(T, datum_t, delete) (TYPE(T, datum_t) deque, void (*destructor) (datum_t)){ datum_t storage;
    while (!METHOD(T, datum_t, is_empty) (deque)){
        if (destructor){destructor(METHOD(T, datum_t, first) (deque));} // If the datum needs to be freed we use the destructor given in parameters
        
        deque = METHOD(T, datum_t, pop_front)(&storage, deque); // Use of pop_back so the deque head doesn't change
    }
...
void METHOD(T, datum_t, print) (TYPE(T, datum_t) deque){
    printf("{ ");
    if (METHOD(T, datum_t, is_empty)(deque)){printf("}\n"); return;}
