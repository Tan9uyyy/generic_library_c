...
/* Retourne le premier element de la queue */
T_QUEUE_INTERFACE queue_datum_t QUEUE_METHOD(first) (T queue);
/* Insere l'element a la fin de la queue et retourne la nouvelle queue */
T_QUEUE_INTERFACE T             QUEUE_METHOD(push) (queue_datum_t value, T queue);
/* Retire le premier element de la queue, le stocke dans value et retourne la nouvelle queue */
T_QUEUE_INTERFACE T             QUEUE_METHOD(pop) (queue_datum_t *value, T queue);
/* Supprime la queue proprement en utilisant le destructeur sur chaque element */
T_QUEUE_INTERFACE void          QUEUE_METHOD(delete) (T queue);
/* Affiche la queue en utilisant la fonction printer pour chaque element */
