...
T_MAP_INTERFACE int MAP_METHOD(remove)(T *hashtable, T_MAP_KEY key) {
  ...
  couple_exemple.key = key;
  int hash_code = HASH(key) % ((*hashtable)->length);
  if (LIST_METHOD(contains)((*hashtable)->buckets[hash_code], couple_exemple) >=
      0) {
...
/* Rajoute le couple key,value dans la hashmap */
T_MAP_INTERFACE int MAP_METHOD(put)(T *hashtable, T_MAP_KEY key,
  ...
  int contains =
      LIST_METHOD(contains)((*hashtable)->buckets[hash_code], new_couple);
  if (contains < 0) {
    (*hashtable)->count++;
    /* on vérifie le load factor avant d'ajouter */
    float current_load_factor =
        (float)(*hashtable)->count / (float)(*hashtable)->length;
  ...
  (*hashtable)->buckets[hash_code] =
      DEQUE_METHOD(push_front)(new_couple, (*hashtable)->buckets[hash_code]);
  return 0;
}
#else
T_MAP_INTERFACE int MAP_METHOD(insert)(T *set, T_SET_ELEMENT element) {
  ...
  COUPLE_TYPE new_couple;
  new_couple.key = element;

  int hash_code = HASH(element) % (hashtable->length);
  int contains =
      LIST_METHOD(contains)(hashtable->buckets[hash_code], new_couple);
  ...
  if (contains < 0) {
    ...
    if (current_load_factor >= LOAD_FACTOR) {
      ...
      for (size_t i = 0; i < hashtable->length; i++) {
        ...
        for (size_t j = 0; j < old_list_length; j++) {
          LIST_METHOD(pop)(&old_couple, old_list);
          MAP_METHOD(insert)(&new_hashtable, old_couple.key);
        }
      }
      MAP_METHOD(delete)(set);
      *set = new_hashtable;
      hashtable = *set;
      hash_code = HASH(element) % (hashtable->length);
    }
    hashtable->buckets[hash_code] =
        DEQUE_METHOD(push_front)(new_couple, hashtable->buckets[hash_code]);
  }
...
T_MAP_INTERFACE int MAP_METHOD(get)(T hashtable, T_MAP_KEY key,
  ...
  *value = saved_couple.value;
  ...
  hashtable->buckets[hash_code] =
      LIST_METHOD(remove)(saved_couple, hashtable->buckets[hash_code]);
  hashtable->buckets[hash_code] =
      DEQUE_METHOD(push_front)(saved_couple, hashtable->buckets[hash_code]);
#endif
  return 0;
}
