...
/* Retourne le nombre d'elements dans le tableau */
size_t array_length(void *array);
#define array_is_empty(array) (0 == array_length(array))

/* Retourne la taille reelle du tableau alloue */
size_t array_size(void *array);

size_t array_resize__(void **array_ptr, size_t nmemb);
#define array_resize(array_ptr, nmemb)                                         \
  array_resize__((void **)(array_ptr), nmemb)

#define array_push(array_ptr, element)                                         \
  (array_resize((array_ptr), array_length(*(array_ptr)) + 1),                  \
   (*(array_ptr))[array_length(*(array_ptr)) - 1] = element)

#define array_pop(array_ptr, element)                                          \
  (*(element) = *(array_ptr) ? (*(array_ptr))[array_length(*(array_ptr)) - 1]  \
                             : *(element),                                     \
   array_resize((array_ptr), array_length(*(array_ptr)) - 1))
