...
TYPE(T, heap_datum_t) METHOD(T, heap_datum_t, new) (TYPE(T, heap_datum_t) heap){return array(heap_datum_t);}

int METHOD(T, heap_datum_t, is_empty) (TYPE(T, heap_datum_t) heap){return array_is_empty(heap);}

int METHOD(T, heap_datum_t, length) (TYPE(T, heap_datum_t) heap){return array_length(heap);}
...
int METHOD(T, heap_datum_t, swap) (TYPE(T, heap_datum_t) heap, int src, int dst){
    assert((src < array_length(heap)) && (dst < array_length(heap)) && "Index out of range !");
...
int METHOD(T, heap_datum_t, push) (TYPE(T, heap_datum_t) heap, heap_datum_t value){
    int index = array_length(heap);
    array_push(&heap, value);

    while (heap[index] > heap[FATHER(index)] && index > 0){
...
int METHOD(T, heap_datum_t, pop) (TYPE(T, heap_datum_t) heap, heap_datum_t* value){
    assert(!array_is_empty(heap) && "Heap is empty !");

    METHOD(T, heap_datum_t, swap) (heap, 0, array_length(heap)-1);
    array_pop(&heap, value);

    int index = 0; int len = array_length(heap);
    while (HAS_SON(index, len)){
        if (RIGHT_SON(index) > len) METHOD(T, heap_datum_t, swap) (heap, index, LEFT_SON(index));
        if (heap[RIGHT_SON(index)] > heap[LEFT_SON(index)]) METHOD(T, heap_datum_t, swap) (heap, index, RIGHT_SON(index));
