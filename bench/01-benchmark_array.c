...
//printf( "%10zu\t,", N/2 );
    ...
    for(int run = 0 ; run < runs ; run++)
        s1 = run + onarray( N ); /* Perform test for dynamic array column */
  ...
  for ( int run = 0 ; run < runs ; run++ )
    s3 = run + onheap( N ); /* Perform test for onheap column */
...
int main () {
  ...
  /* Print CSV header */
  printf( " log2(N)   \t,  dynamic array      \t,  Op2      \t,  onheap     \t, onstack   \t\n" );
